-- fn_claim_referral_queue_to_enrollment returns TABLE(enrollment_id uuid, ...).
-- Unqualified "enrollment_id" in WHERE then conflicts with that OUT name (42702).
...
begin
  ...
  if v_navigator_person_id is not null then
    update atlas.navigator_assignments na
     ...
     where na.enrollment_id = v_enrollment_id
       and na.ends_on is null
       and na.navigator_person_id <> v_navigator_person_id;
