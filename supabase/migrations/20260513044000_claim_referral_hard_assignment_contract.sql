-- Enforce a strict claim-to-assignment contract:
-- - claiming must resolve to the authenticated navigator
-- - no fallback assignment to "latest navigator"
-- - claim fails if we cannot hard-assign the resulting enrollment
...
begin
  ...
  -- Use the canonical self-assignment contract so claim cannot succeed unless
  -- the active auth session maps to an eligible navigator identity.
  perform atlas.fn_navigator_assign_enrollment_to_self(v_enrollment_id);
...
revoke all on function atlas.fn_claim_referral_queue_to_enrollment(text, text, text, text, text, text, text, text, jsonb) from public;
