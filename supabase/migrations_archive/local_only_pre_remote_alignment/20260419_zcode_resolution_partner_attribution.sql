alter table atlas.enrollee_z_codes
  add column if not exists resolution_partner_id uuid references atlas.partners(id) on delete set null,
  add column if not exists resolution_note text;

drop function if exists atlas.fn_set_enrollee_z_code_resolution(uuid, boolean);
...
grant execute on function atlas.fn_set_enrollee_z_code_resolution(uuid, boolean, uuid, text) to anon, authenticated;
