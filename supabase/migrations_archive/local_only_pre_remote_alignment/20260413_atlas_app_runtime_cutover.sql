...
create table if not exists atlas.route_builder_bom_items (
  id text primary key,
  title text not null,
...
create table if not exists atlas.route_builder_steps (
  id text primary key,
  bom_item_id text not null references atlas.route_builder_bom_items(id) on delete cascade,
  label text not null,
...
create table if not exists atlas.route_builder_templates (
  id text primary key,
  name text not null,
  description text not null,
  target_phase text not null check (target_phase in ('regulation', 'readiness', 'renewal')),
  bom_item_ids text[] not null default '{}',
...
create table if not exists atlas.route_builder_journey_assignments (
  id text primary key,
  enrollment_id uuid not null references atlas.enrollments(id) on delete cascade,
  template_id text not null references atlas.route_builder_templates(id) on delete restrict,
  step_ids text[] not null default '{}',
...
create table if not exists atlas.legacy_atlas_participants (
  participant_id text primary key,
  county_id text,
  display_name text not null,
  current_phase text not null check (current_phase in ('Regulation', 'Readiness', 'Renewal')),
  phase_readiness numeric(6,4) not null default 0,
  pressure_vectors jsonb not null default '[]'::jsonb,
...
create table if not exists atlas.legacy_atlas_capacity_nodes (
  partner_id text primary key,
  label text not null,
  route_class text not null,
  coverage_score numeric(6,4) not null default 0,
  phase_alignment numeric(6,4) not null default 0,
  specialization_score numeric(6,4) not null default 0,
  reversibility_support numeric(6,4) not null default 0,
  transfer_cost numeric(6,4) not null default 0,
  interference_risk numeric(6,4) not null default 0,
  phase_index int not null default 0,
  domain text,
  primary_domain text,
  domain_coverage text[] not null default '{}',
  blockers text[] not null default '{}',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create table if not exists atlas.legacy_atlas_routes (
  id uuid primary key default gen_random_uuid(),
  route_id text not null,
  participant_id text not null references atlas.legacy_atlas_participants(participant_id) on delete cascade,
  partner_id text,
  route_class text not null,
  status text not null,
  score numeric(8,4),
  interference_risk numeric(8,4),
  transfer_cost numeric(8,4),
...
create table if not exists atlas.legacy_atlas_route_steps (
  id uuid primary key default gen_random_uuid(),
  route_record_id uuid references atlas.legacy_atlas_routes(id) on delete cascade,
  route_id text not null,
  participant_id text not null references atlas.legacy_atlas_participants(participant_id) on delete cascade,
  partner_id text,
...
create table if not exists atlas.legacy_atlas_memory_events (
  id uuid primary key default gen_random_uuid(),
  participant_id text not null references atlas.legacy_atlas_participants(participant_id) on delete cascade,
...
create table if not exists atlas.legacy_atlas_ontology_weights (
  config_key text primary key default 'default',
  coverage_weight numeric(6,4) not null default 0.3,
  phase_alignment_weight numeric(6,4) not null default 0.2,
  specialization_weight numeric(6,4) not null default 0.2,
  reversibility_weight numeric(6,4) not null default 0.15,
  transfer_cost_penalty numeric(6,4) not null default 0.1,
  interference_penalty numeric(6,4) not null default 0.05,
  civic_diplomacy_boost numeric(6,4) not null default 0.08,
  sla_threshold_hours int not null default 48,
  interference_medium_threshold numeric(6,4) not null default 0.35,
  interference_high_threshold numeric(6,4) not null default 0.6,
  phase_readiness_alert_threshold numeric(6,4) not null default 0.45,
  pcf_refinement_weight numeric(6,4) not null default 0.6,
  reciprocity_activation_threshold numeric(6,4) not null default 0.6,
  updated_at timestamptz not null default now()
);

create table if not exists atlas.legacy_atlas_ontology_audit (
  id uuid primary key default gen_random_uuid(),
  event_type text,
  actor_role text,
  actor_user_id text,
...
create table if not exists atlas.legacy_atlas_renewal_roles (
  participant_id text primary key references atlas.legacy_atlas_participants(participant_id) on delete cascade,
  role_label text,
...
drop trigger if exists trg_route_builder_bom_items_touch_updated_at on atlas.route_builder_bom_items;
create trigger trg_route_builder_bom_items_touch_updated_at
before update on atlas.route_builder_bom_items
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_route_builder_steps_touch_updated_at on atlas.route_builder_steps;
create trigger trg_route_builder_steps_touch_updated_at
before update on atlas.route_builder_steps
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_route_builder_templates_touch_updated_at on atlas.route_builder_templates;
create trigger trg_route_builder_templates_touch_updated_at
before update on atlas.route_builder_templates
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_route_builder_journey_assignments_touch_updated_at on atlas.route_builder_journey_assignments;
create trigger trg_route_builder_journey_assignments_touch_updated_at
before update on atlas.route_builder_journey_assignments
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_participants_touch_updated_at on atlas.legacy_atlas_participants;
create trigger trg_legacy_atlas_participants_touch_updated_at
before update on atlas.legacy_atlas_participants
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_capacity_nodes_touch_updated_at on atlas.legacy_atlas_capacity_nodes;
create trigger trg_legacy_atlas_capacity_nodes_touch_updated_at
before update on atlas.legacy_atlas_capacity_nodes
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_routes_touch_updated_at on atlas.legacy_atlas_routes;
create trigger trg_legacy_atlas_routes_touch_updated_at
before update on atlas.legacy_atlas_routes
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_route_steps_touch_updated_at on atlas.legacy_atlas_route_steps;
create trigger trg_legacy_atlas_route_steps_touch_updated_at
before update on atlas.legacy_atlas_route_steps
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_ontology_weights_touch_updated_at on atlas.legacy_atlas_ontology_weights;
create trigger trg_legacy_atlas_ontology_weights_touch_updated_at
before update on atlas.legacy_atlas_ontology_weights
for each row execute function atlas.fn_touch_updated_at();

drop trigger if exists trg_legacy_atlas_renewal_roles_touch_updated_at on atlas.legacy_atlas_renewal_roles;
create trigger trg_legacy_atlas_renewal_roles_touch_updated_at
before update on atlas.legacy_atlas_renewal_roles
for each row execute function atlas.fn_touch_updated_at();
...
create or replace view atlas.v_route_builder_participants as
select
  en.id as participant_id,
  p.display_name as participant_name,
  c.county_name as county_name,
  e.current_phase,
  case e.current_phase
    when 'renewal' then 0.82
    when 'readiness' then 0.62
    else 0.38
  end::numeric(6,4) as readiness_score,
  rja.id as active_journey_id
from atlas.enrollments en
join atlas.enrollees e on e.id = en.enrollee_id
join atlas.people p on p.id = e.person_id
left join atlas.counties c on c.id = e.county_id
left join atlas.route_builder_journey_assignments rja
  on rja.enrollment_id = en.id
...
values
  ...
  (
    ...
    '2026-v1',
    '{
      "roles": ["peerNavigator", "stationOperator", "regionalDirector", "governanceAdmin", "readOnlyFunder"],
      "policyBoundaries": {
        "evaluateParticipant": ["peerNavigator", "stationOperator", "regionalDirector"],
        "activateRoute": ["peerNavigator", "stationOperator"],
        "transitionRoute": ["peerNavigator", "stationOperator"],
        "appendMemoryEvent": ["peerNavigator", "stationOperator", "regionalDirector"],
        "assignRenewalRole": ["peerNavigator", "stationOperator", "regionalDirector", "governanceAdmin"],
        "viewSystemHeatmap": ["peerNavigator", "stationOperator", "regionalDirector", "readOnlyFunder"],
        "manageOntology": ["governanceAdmin"]
      },
      "stabilizationPhases": ["Regulation", "Readiness", "Renewal"],
      "pressureDomains": [
      ...
      "reciprocityEthos": "This community takes care of me - and I take care of it.",
      "srigCoordinationAreas": ["policyGovernance", "institutionalMaturation", "stabilizationPathways", "civicDiplomacy", "earlyInterventionRouting", "renewalStandards"],
...
insert into atlas.route_builder_bom_items (id, title, domain, description, required, default_duration_days)
values
  ('bom-safety-triage', 'safety triage block', 'stabilization', 'confirm immediate risks, de-escalate pressure, and lock first safe contact.', true, 2),
  ('bom-housing-anchor', 'housing anchor block', 'habitat', 'secure housing continuity and remove active displacement threats.', true, 7),
  ('bom-employment-rhythm', 'employment rhythm block', 'work', 'rebuild routine through stable work access and schedule design.', false, 10),
  ('bom-civic-contribution', 'civic contribution block', 'renewal', 'assign participant to a reciprocity role with verifiable contribution receipts.', false, 14)
...
insert into atlas.route_builder_steps (id, bom_item_id, label, phase, instruction, owner_role, exit_criteria, sequence)
values
  ('step-001', 'bom-safety-triage', 'stabilize immediate risk', 'regulation', 'confirm safe location, remove active threat, establish 24-hour check-in cadence.', 'peer navigator', 'no active threat escalation in 24 hours.', 1),
  ('step-002', 'bom-housing-anchor', 'secure housing continuity', 'readiness', 'activate housing partner and confirm temporary to stable unit path.', 'station operator', 'housing continuity confirmed for 14 days.', 2),
  ('step-003', 'bom-employment-rhythm', 'restore work rhythm', 'readiness', 'set weekly schedule and connect with workforce support.', 'regional director', 'participant reports predictable routine for 2 consecutive weeks.', 3),
  ('step-004', 'bom-civic-contribution', 'activate civic reciprocity role', 'renewal', 'assign renewal role and capture first verified contribution receipt.', 'governance admin', 'verified receipt logged and participant role active.', 4)
on conflict (id) do update
...
insert into atlas.route_builder_templates (id, name, description, target_phase, bom_item_ids, step_ids, is_core)
values
  ('template-fast-stabilization', 'fast stabilization', 'short sequence to move from threat control to readiness posture.', 'readiness', '{bom-safety-triage,bom-housing-anchor}', '{step-001,step-002}', true),
  ('template-social-restoration', 'social restoration', 'readiness through social/work rhythm and renewal activation.', 'renewal', '{bom-housing-anchor,bom-employment-rhythm,bom-civic-contribution}', '{step-002,step-003,step-004}', true)
on conflict (id) do update
...
insert into atlas.legacy_atlas_participants (participant_id, county_id, display_name, current_phase, phase_readiness, pressure_vectors)
values
  (
    'pt-101',
    'county-commons-01',
    'North District Participant',
    'Regulation',
    0.35,
    '[
...
insert into atlas.legacy_atlas_capacity_nodes (
  partner_id, label, route_class, coverage_score, phase_alignment, specialization_score, reversibility_support,
  transfer_cost, interference_risk, phase_index
)
values
  ('station-housing-01', 'Housing Stabilization Station', 'stabilization', 0.84, 0.89, 0.82, 0.78, 0.22, 0.16, 0),
  ('station-work-03', 'Work Restoration Station', 'readiness', 0.74, 0.75, 0.86, 0.71, 0.25, 0.2, 1),
  ('station-network-02', 'Social Network Repair Station', 'civicDiplomacy', 0.78, 0.8, 0.72, 0.66, 0.31, 0.27, 2)
...
insert into atlas.legacy_atlas_ontology_weights (config_key)
values ('default')
...
grant select, insert, update, delete on atlas.route_builder_bom_items to anon, authenticated;
grant select, insert, update, delete on atlas.route_builder_steps to anon, authenticated;
grant select, insert, update, delete on atlas.route_builder_templates to anon, authenticated;
grant select, insert, update, delete on atlas.route_builder_journey_assignments to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_participants to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_capacity_nodes to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_routes to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_route_steps to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_memory_events to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_ontology_weights to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_ontology_audit to anon, authenticated;
grant select, insert, update, delete on atlas.legacy_atlas_renewal_roles to anon, authenticated;
...
grant select on atlas.v_route_builder_participants to anon, authenticated;
