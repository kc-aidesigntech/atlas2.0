// Civic bioengineering snapshots translate participant progress into ascent, renewal, and ecosystem signals.
import {
...
export function buildAscentEngineSnapshot({
  participant,
  routes = [],
  steps = [],
  ...
  const completedRouteRatio = routes.length
    ...
    : 0
  const completedStepRatio = steps.length
    ? steps.filter((step) => step.status === STEP_STATUS.completed).length / steps.length
    : 0
...
export function buildInstitutionalEcosystemSnapshot({ isLiveData }: { isLiveData: boolean }) {
