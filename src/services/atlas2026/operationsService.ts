// Operations service aggregates cross-participant workload, risk, readiness, and county metrics.
...
import {
  ...
  toRoundedNumber
} from '@/services/atlas2026/snapshotHelpers'
import type { MemoryEventSnapshot, ParticipantSnapshot, TimestampLike } from '@/services/atlas2026/snapshotHelpers'
import { STEP_STATUS } from '@/services/atlas2026/stepGraph'
import type { RouteStep } from '@/services/atlas2026/stepGraph'

interface OperationsRoute {
  participantId?: string
  status?: string
  [key: string]: unknown
}

interface OperationsStep extends RouteStep {
...
interface OperationsInput {
  ...
  memoryEvents: MemoryEventSnapshot[]
...
export function buildOperationsSnapshot({
  ...
  const overdueSteps = stepAges.filter(
    (step) =>
      [STEP_STATUS.pending, STEP_STATUS.inProgress, STEP_STATUS.blocked].some((status) => status === step.status) &&
      step.ageHours >= slaThresholdHours
  )
  const avgStepAgeHours = stepAges.length
    ? stepAges.reduce((sum, step) => sum + step.ageHours, 0) / stepAges.length
    : 0
  const participantCountyById = new Map(
    participants.map((participant) => [participant.participantId, participant.countyId || 'unknown'])
  )
...
export function buildCountyComparisonSnapshot({
  participants,
  routes,
  steps,
  memoryEvents,
  ...
  return counties.map((countyId) => {
    ...
    const readinessAvg = scopedParticipants.length
      ? scopedParticipants.reduce((sum, participant) => sum + (participant.phaseReadiness || 0), 0) /
