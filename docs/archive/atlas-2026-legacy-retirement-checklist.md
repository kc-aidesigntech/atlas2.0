...
- [x] Runtime data access moved to Supabase; Firebase client and `firestore.rules` removed from the web app surface.
...
- [ ] Validate Supabase Row-Level Security (RLS) behavior for all five roles.

## Post-Cutover Cleanup Tasks

- [ ] Remove leftover Firebase seed scripts under `scripts/` after confirming Supabase seeding is the only path.
