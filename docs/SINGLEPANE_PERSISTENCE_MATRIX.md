...
| Regulation test history (`MH-SCA`, `SVS`) | `atlas.navigator_regulation_test_submissions` + `atlas.navigator_regulation_test_answers` | submission id / `draft_key` | insert historical attempts and update drafts; completed rows become strip-map gate inputs | `loadRegulationTestHistory()` -> `useSinglePaneData()` | persisted contract active |
| Regulation gate / strip-map readiness visibility | read-only derived from latest completed regulation tests per enrollee | `enrollee_id` at query time | no direct write; recomputed from persisted test history every reload | `useSinglePaneData()` -> strip map components + route-planning gate | persisted derived read contract active |
| Route planning candidate ranking / ordering | read-only derived from `atlas.partner_z_code_burden_scores` + active `atlas.enrollee_z_codes` | `enrollment_id` at query time | no direct write; partner survey completion refreshes partner burden rows and active enrollee Z-codes change the read result | `atlas.fn_rank_route_candidates()` -> `atlas.v_navigator_route_candidates` -> `loadRouteCandidates()`; now includes per-parent summaries for the symbolic mobile route board | persisted derived read contract active |
