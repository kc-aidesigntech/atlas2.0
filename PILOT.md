...
- Browser automation is wired up via Playwright (`pilot-decks/capture/`) and was
  ...
  (`verification/pilot_verify.sql`) remains the authoritative per-credential
  alignment check; the browser pipeline is for demonstration/screenshots.

---

## 9. Pilot decks (slides)

Two sets of themed slide decks accompany this guide, in `pilot-decks/`:

- **Exploration plan** (`Atlas-Pilot-0{1..4}-*.pptx`) — text-only step-by-step plan
  per role. Generated by `pilot-decks/generate_pilot_decks.py`.
- **Walkthrough** (`Atlas-Pilot-Walkthrough-0{1..4}-*.pptx`) — the same steps paired
  with **real screenshots** captured from the live app. Generated by
  `pilot-decks/generate_pilot_decks_shots.py`.

Regenerate the walkthrough decks end to end (requires `npm run dev` running):

```bash
# one-time: Playwright + its browser (kept out of git via .gitignore)
python3 -m pip install --user playwright python-pptx
PLAYWRIGHT_BROWSERS_PATH="$PWD/.playwright-browsers" python3 -m playwright install chromium

# capture screenshots, then build the decks
PLAYWRIGHT_BROWSERS_PATH="$PWD/.playwright-browsers" python3 pilot-decks/capture/capture_screens.py
python3 pilot-decks/generate_pilot_decks_shots.py
```

The capture script signs in as each pilot identity, sets the role switcher, and
screenshots the real screen behind every step into `pilot-decks/screenshots/`.
